...
class ClientAssessmentInitializationTests (TestCase):
    ...
    def test_entry_assessment_is_constructed_when_no_member_is_set(self):
        models.ClientEntryAssessment()

    def test_entry_date_is_copied(self):
        from datetime import date
        # Load a household member with no entry date yet set.
        member = models.HouseholdMember.objects.get(id=19)
        # Set the Entry date.
        member.entry_date = date(2014, 12, 6)
        # Ensure that any new entry_assessments respect the entry date.
        entry_assessment = models.ClientEntryAssessment(member=member)

        assert entry_assessment.project_entry_date == date(2014, 12, 6)

    def test_explict_entry_date_is_respected(self):
        from datetime import date
        # Load a household member with no entry date yet set.
        member = models.HouseholdMember.objects.get(id=19)
        # Set the Entry date.
        member.entry_date = date(2014, 12, 6)
        # Ensure that any new entry_assessments respect the entry date.
        entry_assessment = models.ClientEntryAssessment(
            member=member, project_entry_date=date(2015, 1, 12))

        assert entry_assessment.project_entry_date == date(2015, 1, 12)

    def test_exit_assessment_is_constructed_when_no_member_is_set(self):
        models.ClientExitAssessment()

    def test_exit_date_is_copied(self):
        from datetime import date
        # Load a household member with no exit date yet set.
        member = models.HouseholdMember.objects.get(id=19)
        # Set the exit date.
        member.exit_date = date(2014, 12, 6)
        # Ensure that any new exit_assessments respect the exit date.
        exit_assessment = models.ClientExitAssessment(member=member)

        assert exit_assessment.project_exit_date == date(2014, 12, 6)

    def test_explict_exit_date_is_respected(self):
        from datetime import date
        # Load a household member with no exit date yet set.
        member = models.HouseholdMember.objects.get(id=19)
        # Set the exit date.
        member.exit_date = date(2014, 12, 6)
        # Ensure that any new exit_assessments respect the exit date.
        exit_assessment = models.ClientExitAssessment(
            member=member, project_exit_date=date(2015, 1, 12))

        assert exit_assessment.project_exit_date == date(2015, 1, 12)


class ClientAssessmentSaveTests (TestCase):
    fixtures = ['staff-groups', 'hmis-test-data']
